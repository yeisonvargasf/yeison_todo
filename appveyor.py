...
This file is used to build and distribute the yeison_todo binary on appveyor. Take
...
class environment:
    ...
    def dist(self):
        ...
        for arch, python in self.python:
            ...
            # Build the binary
            build_path = os.path.join("dist", f"yeison_todo-{arch}")
            self.run(f"{python} -m PyInstaller yeison_todo.spec"
            ...
            artifact_path = os.path.join(
                os.getcwd(),
                "dist",
                f"yeison_todo-{self.os}-{'i686' if arch == 32 else 'x86_64'}"
            )
            binary_path = os.path.join(os.getcwd(), build_path, "yeison_todo")
            if self.os == self.WIN:
